          ...
          return colArgsList.all().map(function(arg1, ci) {
            ...
            return R.tr({}, _.flatten([
              rearrangeable ? R.th({
                ...
                results = [];
                for (var i = 0, ref = cols.get().num / (cols.get().widths[ci] * values.length); 0 <= ref ? i < ref : i > ref; 0 <= ref ? i++ : i--){ results.push(i); }
                return results;
          ...
          if (rowArgsList.length() > 1 && rearrangeable) {
            return R.tr({}, rx.flatten([
              (function() {
                ...
                results1 = [];
                for (var j = 0, ref1 = cols.get().values.length; 0 <= ref1 ? j < ref1 : j > ref1; 0 <= ref1 ? j++ : j--){ results1.push(j); }
                return results1;
              }).apply(this).map(function() {
                return R.th({
                  style: {
